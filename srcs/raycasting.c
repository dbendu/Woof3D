...
/*   Updated: 2020/11/02 16:06:49 by mburl            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_ray			*raycast(const float pov, t_position start, t_vector_point *map)
{
...
t_ray			*raycast(float pov, t_position start, t_vector_point *map)
{
	...
	float start_ = pov + HERO_FOV / 2.0f;
	if (start_ > 360.0f) {
		start_ -= 360.0f;
	}

	float distance = fabsf(WND_WIDTH / (2.0f * tanf(HERO_FOV / 2.0f)));
	float delta = HERO_FOV / (float)WND_WIDTH;
	float *angles = malloc(sizeof(float) * WND_WIDTH);
	for (int i = 0; i < WND_WIDTH; ++i) {
		angles[i] = start_ - to_ang(atanf((WND_WIDTH / 2 - i) * delta / distance));
	}
//	for (int i = WND_WIDTH / 2; i >= 0; --i) {
//		angles[i] = start_ - to_ang(atanf((WND_WIDTH / 2 - i) * delta / distance));
//	}
//	for (int i = WND_WIDTH / 2; i < WND_WIDTH; ++i) {
//		angles[i] = pov - to_ang(atanf((i - WND_WIDTH / 2) * delta / distance));
//	}
	if (first) {
		for (int i = 0; i < WND_WIDTH; ++i) {
			printf("%d: %f\n", i, angles[i]);
